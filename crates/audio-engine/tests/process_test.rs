...
#[test]
fn clip_start_and_stop_are_sample_accurate_within_a_block() {
    let format = AudioFormat::new(48_000, 1).expect("valid mono format");
    let mut graph =
        AudioGraph::with_oscillator(format, 512, 440.0).expect("graph buffers are valid");
    let mut timeline = Timeline::new();
    timeline.add_clip(AudioClip::new(
        ClipId::new(1),
        TrackId::new(1),
    ...
    graph.set_snapshot(AudioSnapshot::compile(&timeline).expect("valid timeline compiles"));
    let mut output = AudioBuffer::new(512, format).expect("buffer dimensions are valid");
...
#[test]
fn overlapping_clips_change_the_active_signal_at_exact_offsets() {
    let format = AudioFormat::new(48_000, 1).expect("valid mono format");
    let mut graph =
        AudioGraph::with_oscillator(format, 512, 440.0).expect("graph buffers are valid");
    let mut timeline = Timeline::new();
